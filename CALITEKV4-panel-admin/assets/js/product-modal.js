(() => {
  ...
  function getPricesByServiceInternal(basePrice, transportTaxes, quantity, unit = "g") {
    ...
    // Utiliser directement les prix depuis transportTaxes, même si basePrice est 0
    // Si un prix n'est pas défini dans transportTaxes, utiliser basePrice comme fallback
    const homePrice = priceEntry.services.home !== undefined ? parseFloat(priceEntry.services.home) : basePrice;
    const postalPrice = priceEntry.services.postal !== undefined ? parseFloat(priceEntry.services.postal) : basePrice;
    const meetPrice = priceEntry.services.meet !== undefined ? parseFloat(priceEntry.services.meet) : basePrice;
    
    return {
      home: isNaN(homePrice) ? basePrice : homePrice,
      postal: isNaN(postalPrice) ? basePrice : postalPrice,
      meet: isNaN(meetPrice) ? basePrice : meetPrice
    };
  }
  ...
  function formatPrice(price, transportTaxes, quantity, config, unit = "g", selectedService = null) {
    ...
    // Détecter si on est sur mobile (défini avant les conditions)
    const isMobile = window.innerWidth <= 768;
    const separator = isMobile ? " " : " / ";
    
    // Afficher tous les prix configurés dans transportTaxes (même si 0 ou égal au prix de base)
    // Les valeurs dans transportTaxes sont des prix finaux, donc on les affiche directement
    // On vérifie si on a des transportTaxes configurés pour cette quantité
    const hasTransportTaxes = transportTaxes && transportTaxes.length > 0;
    
    if (hasTransportTaxes) {
      // Normaliser la quantité pour la recherche
      const quantityNum = typeof quantity === 'string' ? parseFloat(quantity.replace(/[^\d.]/g, '')) : quantity;
      const quantityStr = `${quantityNum}${unit}`;
      const quantityNumStr = String(quantityNum);
      
      // Chercher la configuration de prix correspondante
      const priceEntry = transportTaxes.find(t => {
        if (!t.quantity) return false;
        const tQty = String(t.quantity).trim();
        return tQty === quantityStr || tQty === quantityNumStr || 
      ...
      if (priceEntry && priceEntry.services) {
        // Afficher tous les prix configurés pour cette quantité (même si 0)
        // On affiche seulement ceux qui sont définis (pas undefined)
        if (priceEntry.services.home !== undefined && priceEntry.services.home !== null) {
          const homePrice = parseFloat(priceEntry.services.home) || 0;
          priceList.push(isMobile ? `🚚${homePrice.toFixed(0)}€` : `🚚 ${homePrice.toFixed(0)}€`);
        }
        if (priceEntry.services.postal !== undefined && priceEntry.services.postal !== null) {
          const postalPrice = parseFloat(priceEntry.services.postal) || 0;
          priceList.push(isMobile ? `📦${postalPrice.toFixed(0)}€` : `📦 ${postalPrice.toFixed(0)}€`);
        }
        if (priceEntry.services.meet !== undefined && priceEntry.services.meet !== null) {
          const meetPrice = parseFloat(priceEntry.services.meet) || 0;
          priceList.push(isMobile ? `📍${meetPrice.toFixed(0)}€` : `📍 ${meetPrice.toFixed(0)}€`);
        }
      } else {
        // Si pas de correspondance exacte, utiliser les prix calculés
        // Afficher seulement ceux qui sont > 0 ou différents du prix de base
        if (prices.home > 0 || (price === 0 && prices.home >= 0)) {
          priceList.push(isMobile ? `🚚${prices.home.toFixed(0)}€` : `🚚 ${prices.home.toFixed(0)}€`);
        }
        if (prices.postal > 0 || (price === 0 && prices.postal >= 0)) {
          priceList.push(isMobile ? `📦${prices.postal.toFixed(0)}€` : `📦 ${prices.postal.toFixed(0)}€`);
        }
        if (prices.meet > 0 || (price === 0 && prices.meet >= 0)) {
          priceList.push(isMobile ? `📍${prices.meet.toFixed(0)}€` : `📍 ${prices.meet.toFixed(0)}€`);
        }
      }
    } else {
      // Fallback : afficher les prix calculés si > 0
      if (prices.home > 0) priceList.push(isMobile ? `🚚${prices.home.toFixed(0)}€` : `🚚 ${prices.home.toFixed(0)}€`);
      if (prices.postal > 0) priceList.push(isMobile ? `📦${prices.postal.toFixed(0)}€` : `📦 ${prices.postal.toFixed(0)}€`);
      if (prices.meet > 0) priceList.push(isMobile ? `📍${prices.meet.toFixed(0)}€` : `📍 ${prices.meet.toFixed(0)}€`);
    }
    
    if (priceList.length > 0) {
      return priceList.join(separator);
    }
  ...
  window.showProductModal = function(product) {
    ...
    // Déclencher l'ajout des décorations de fête si nécessaire
    if (window.addDecorationsToModals) {
      setTimeout(() => {
        window.addDecorationsToModals();
      }, 300);
    }
  };
